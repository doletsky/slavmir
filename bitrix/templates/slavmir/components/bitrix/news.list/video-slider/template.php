...
<section class="daily_video_slider_box">
	<div class="daily_video_slider">
		<?foreach( $arResult["ITEMS"] as $arItem ){
			...
			?>
			<div class="daily_video_item pl-video-play <?if($needBlock){?>block<?}?>" data-name="<?=$arItem["NAME"]?>" data-artist="<?=$artist?>" data-picture="<?=$image?>" data-url="<?=$url?>">
				<div class="item_img" style="background-image: url(<?=$image?>);"></div>
				<div class="play_video_btn"><img src="<?=SITE_TEMPLATE_PATH?>/images/play_btn_large.png" alt="<?=$arItem["NAME"]?>"></div>
